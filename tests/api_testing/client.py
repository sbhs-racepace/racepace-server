...
class TestApiClient:
    ...
    def register_user(self, **credentials):
        ...
        if resp["success"]:
            print(
                "Created user with email: {email} and password: {password}".format(
                    **credentials
                )
            )
        else:
    ...
    def get_route(self, start, end):
        response = requests.get(
            self.BASE + "/route", params={"start": start, "end": end}
        )
    ...
    def get_multiple_route(self, waypoints):
        response = requests.get(
            self.BASE + "/route/multiple", params={"waypoints": waypoints}
        )
        return response.json()

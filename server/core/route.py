...
class RealTimeRoute:
    ...
    def from_data(cls, data):
        ...
        location_history = [
            LocationPacket(
                location_packet.latitude,
                location_packet.longitude,
                location_packet.time,
            )
            for location_packet in json_location_history
        ]
        real_time_route = cls(
            start_time,
            location_history,
    ...
    def update_location_history(self, latitude, longitude, time):
        ...
        if len(self.location_history) >= 1:
            ...
            self.current_distance += (
                current_location_packet.location - previous_location_packet.location
            )
            self.current_duration += (
                current_location_packet.time - previous_location_packet.time
            )
        ...
        if self.current_duration < period:
            ...
            for index in reversed(range(len(self.location_history))):
                ...
                if (self.most_recent_time - location_packet.time) >= period:
                    ...
                    locations = [
                        location_packet.location for location_packet in location_packets
                    ]
    ...
    def to_dict(self):
        return {
            "location_history": [
                location_packet.to_dict() for location_packet in self.location_history
            ],
            "start_time": None
            if self.start_time is None
            else self.start_time.to_dict(),
...
class RecentRoute:
    ...
    def from_real_time_route(cls, real_time_route):
        ...
        """
        points = run_stats(
            real_time_route.current_distance, real_time_route.current_duration
        )
...
class SavedRoute(RecentRoute):
    ...
    def __init__(
        self, route_id, name, description, real_time_route, route_image, points
    ):
    ...
    def from_real_time_route(cls, name, description, route_image, real_time_route):
        ...
        saved_route = cls(
            route_id, name, description, real_time_route, route_image, points
        )
        return saved_route
